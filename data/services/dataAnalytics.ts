import { ServiceConfig } from "./services";

export const dataAnalytics: ServiceConfig = {
  slug: "data-analytics",
  banner: {
    title: "Unlock Business Growth with",
    highlight: "Advanced Data Analytics Solutions",
    subtitle:
      "Transform raw data into actionable insights with our expert analytics services. We help you make data-driven decisions using AI, machine learning, and visualization tools.",
    image:
      "https://cdn.pixabay.com/photo/2024/04/27/07/24/ai-generated-8723288_1280.jpg",
    linkText: "Get Data Analytics Solutions",
  },

  developerDetails: {
    heading: "Expert Data Analysts & BI Engineers",
    subheading:
      "Our analytics experts help organizations leverage data for smarter decisions through BI dashboards, predictive models, and AI-driven insights.",
    mainTitle: "Professional Data Analytics & BI Development Services",
    mainDescription:
      "We specialize in building analytics solutions that uncover trends, predict outcomes, and optimize operations. From data warehousing to AI modeling, we empower your business with data intelligence.",
    image: "/images/data-analytics-dashboard.png",
    services: [
      {
        id: 1,
        title: "Data Visualization & Reporting",
        description:
          "Create interactive dashboards using Power BI, Tableau, or Looker. Present KPIs, trends, and forecasts visually. Enable real-time data monitoring. Improve transparency across departments.",
        icon: "/icons/data-visualization.svg",
      },
      {
        id: 2,
        title: "Predictive Analytics & Machine Learning",
        description:
          "Use AI and ML to predict market trends and customer behavior. Build models for forecasting and risk analysis. Improve decision-making accuracy. Automate repetitive analytics tasks. ",
        icon: "/icons/predictive-analytics.svg",
      },
      {
        id: 3,
        title: "Data Integration & Warehousing",
        description:
          "Consolidate data from multiple sources into a centralized warehouse. Ensure consistency, quality, and accuracy. Enable faster querying and reporting. Implement ETL pipelines for real-time updates. ",
        icon: "/icons/data-warehouse.svg",
      },
    ],
  },

  solutions: {
  mainTitle: "Comprehensive Data Analytics Solutions",
  mainDescription:
    "We deliver end-to-end data analytics and business intelligence services, helping organizations transform data into valuable insights for strategic decision-making and growth.",
  linkText: "Get Data Insights",
  linkUrl: "/contact-us",
  items: [
    {
      id: "data-visualization",
      icon: "Leaf",
      title: "Data Visualization",
      subtitle: "Interactive Dashboards & Reports",
      description:
        "Design dynamic and intuitive dashboards using Power BI and Tableau. \nShowcase real-time KPIs and business metrics with clarity. \nEmpower stakeholders through visual storytelling. \nIdentify trends, gaps, and opportunities quickly. \nDrive better decisions with interactive, data-rich visuals.",
    },
    {
      id: "predictive-modeling",
      icon: "Leaf",
      title: "Predictive Modeling",
      subtitle: "AI-Powered Forecasting",
      description:
        "Build advanced predictive models for accurate forecasting. \nUtilize AI and ML algorithms to analyze complex datasets. \nAnticipate customer needs, risks, and market trends. \nAutomate decisions with data-driven intelligence. \nEmpower growth with proactive business strategies.",
    },
    {
      id: "data-engineering",
      icon: "Leaf",
      title: "Data Engineering",
      subtitle: "Streamlined Data Pipelines",
      description:
        "Integrate, clean, and transform massive data efficiently. \nDevelop scalable ETL and ELT workflows. \nEnsure high data integrity across all sources. \nOptimize performance for analytics and reporting. \nEstablish a robust foundation for advanced data solutions.",
    },
    {
      id: "big-data-solutions",
      icon: "Leaf",
      title: "Big Data Solutions",
      subtitle: "Scalable Analytics Infrastructure",
      description:
        "Leverage Hadoop, Spark, and AWS for large-scale data processing. \nManage structured, semi-structured, and unstructured data. \nEnable real-time analytics and faster computation. \nSupport data-intensive workloads seamlessly. \nUnlock innovation through powerful big data insights.",
    },
    {
      id: "business-intelligence",
      icon: "Leaf",
      title: "Business Intelligence Solutions",
      subtitle: "Data-Driven Decision Support",
      description:
        "Develop comprehensive BI solutions for business visibility. \nAutomate dashboards and performance monitoring. \nReveal actionable insights from operational data. \nEmpower leaders with intuitive data visualization. \nEnhance productivity through smarter analytics tools.",
    },
    {
      id: "data-governance",
      icon: "Leaf",
      title: "Data Governance & Compliance",
      subtitle: "Secure & Reliable Data Management",
      description:
        "Implement governance frameworks for quality and compliance. \nEnsure GDPR, HIPAA, and ISO standard adherence. \nProtect sensitive information with strict access control. \nMaintain transparency through audit-ready systems. \nFoster trust with secure and ethical data practices.",
    },
  ],
},


  managementSolutions: {
    title: "Data Analytics Management Platforms",
    subtitle:
      "Our management solutions empower organizations to collect, analyze, and visualize data seamlessly while ensuring accuracy, security, and accessibility.",
    subtitleLink: { text: "data management tools", url: "#" },
    laptopImage: "/images/data-management-dashboard.jpg",
    mobileImage: "/images/data-management-mobile.jpg",
    topFeatures: [
      {
        id: "data-monitoring",
        icon: "map",
        title: "Real-Time Data Monitoring",
        description:
          "Track business KPIs and operational data in real-time. Identify anomalies quickly. Set automated alerts for performance deviations. Visualize data streams continuously. Empower teams to make proactive business decisions instantly.",
      },
      {
        id: "team-insights",
        icon: "map",
        title: "Collaborative Insights Tools",
        description:
          "Enable teams to share dashboards, analytics, and reports. Improve cross-department collaboration. Facilitate unified data access and comments. Drive collective intelligence and faster decision-making. Enhance data transparency across teams.",
      },
      {
        id: "analytics-reporting",
        icon: "map",
        title: "Advanced Analytics & Reporting",
        description:
          "Generate comprehensive analytics reports. Measure key performance trends. Identify growth opportunities through data exploration. Automate report generation for stakeholders. Improve forecasting accuracy with AI-based analytics.",
      },
    ],
    ctaText: "Get Data Analytics Experts",
    onCtaClick: () => {},
  },

  faq: {
    title: "FAQ - Data Analytics & BI Services",
    items: [
      {
        id: "1",
        question: "What types of data analytics services do you provide?",
        answer:
          "We offer data visualization, predictive analytics, business intelligence, data engineering, and big data solutions to help organizations make smarter decisions.",
      },
      {
        id: "2",
        question: "Can you integrate data from multiple sources?",
        answer:
          "Yes, we integrate data from CRMs, ERPs, APIs, and cloud storage into unified dashboards and warehouses for seamless analytics.",
      },
      {
        id: "3",
        question: "Do you build custom dashboards and reports?",
        answer:
          "Absolutely. We design interactive dashboards using Power BI, Tableau, and Google Data Studio tailored to your specific KPIs and metrics.",
      },
      {
        id: "4",
        question: "Can you implement machine learning for analytics?",
        answer:
          "Yes, our team uses AI and ML algorithms to build predictive models, automate insights, and enhance forecasting accuracy.",
      },
      {
        id: "5",
        question: "How do you ensure data security and privacy?",
        answer:
          "We follow industry standards for encryption, access control, and compliance, ensuring that sensitive business data remains secure.",
      },
      {
        id: "6",
        question: "Do you offer cloud-based analytics solutions?",
        answer:
          "Yes, we deploy analytics platforms on AWS, Azure, and Google Cloud for scalability, speed, and remote accessibility.",
      },
      {
        id: "7",
        question: "Can you provide ongoing support and maintenance?",
        answer:
          "Yes, we offer continuous monitoring, optimization, and enhancement to ensure your analytics systems run efficiently and securely.",
      },
    ],
  },
};
