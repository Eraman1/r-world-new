import { ServiceConfig } from "./services";

export const erpImplementation: ServiceConfig = {
  slug: "erp-implementation",
  banner: {
    title: "Streamline Business Operations with",
    highlight: "ERP Implementation Services",
    subtitle:
      "Transform your business efficiency with customized ERP solutions. We deliver secure, scalable, and integrated systems to manage your core operations seamlessly.",
    image:
      "https://cdn.pixabay.com/photo/2024/04/27/07/24/ai-generated-8723288_1280.jpg",
    linkText: "Get ERP Solutions",
  },

  developerDetails: {
    heading: "Expert ERP Implementation Team",
    subheading:
      "Our ERP specialists plan, design, and implement end-to-end ERP systems to optimize workflows, automate processes, and enhance decision-making across your enterprise.",
    mainTitle: "Professional ERP Implementation Services",
    mainDescription:
      "We provide complete ERP implementation — from needs assessment and customization to integration and training. Our goal is to centralize operations, improve collaboration, and boost productivity using leading ERP platforms like SAP, Oracle, and Odoo.",
    image: "/images/erp-implementation-laptop.png",
    services: [
      {
        id: 1,
        title: "ERP Consulting & Strategy",
        description:
          "Assess your business needs and define a tailored ERP roadmap. Identify the right modules and integrations. Ensure strategic alignment with business goals. Provide cost and ROI analysis. Lay the foundation for a successful ERP journey.",
        icon: "/icons/erp-consulting.svg",
      },
      {
        id: 2,
        title: "ERP Implementation & Integration",
        description:
          "Deploy customized ERP systems across departments. Integrate seamlessly with CRM, HR, finance, and supply chain modules. Ensure smooth data migration and minimal disruption. Optimize workflows for operational efficiency. Provide end-user support and guidance.",
        icon: "/icons/erp-implementation.svg",
      },
      {
        id: 3,
        title: "ERP Customization & Support",
        description:
          "Adapt ERP modules to your specific business requirements. Add features, reports, and automations as needed. Maintain system updates and security patches. Offer continuous technical support. Ensure long-term system reliability and scalability.",
        icon: "/icons/erp-support.svg",
      },
    ],
  },

  solutions: {
  mainTitle: "Comprehensive ERP Implementation Solutions",
  mainDescription:
    "Our ERP services enhance productivity, unify processes, and empower organizations to make data-driven decisions. We help businesses of all sizes achieve efficiency and visibility across all operations.",
  linkText: "Get ERP Solutions",
  linkUrl: "/contact-us",
  items: [
    {
      id: "finance-management",
      icon: "Leaf",
      title: "Financial Management",
      subtitle: "Control & Visibility",
      description:
        "Automate accounting, invoicing, and expense tracking for smooth financial workflows. \nEnsure compliance with global standards and audit requirements. \nGain accurate insights through real-time financial dashboards. \nStreamline budgeting, forecasting, and reconciliation processes. \nEmpower leadership with transparent financial control.",
    },
    {
      id: "inventory-supplychain",
      icon: "Leaf",
      title: "Inventory & Supply Chain",
      subtitle: "Streamlined Operations",
      description:
        "Manage procurement, warehousing, and logistics from a unified platform. \nTrack inventory levels, order cycles, and supplier performance. \nReduce waste, delays, and unexpected shortages. \nAutomate purchasing and replenishment for efficiency. \nEnsure seamless coordination across your entire supply chain.",
    },
    {
      id: "hr-management",
      icon: "Leaf",
      title: "HR & Payroll Management",
      subtitle: "Empower Your Workforce",
      description:
        "Unify employee data, payroll, and compliance in one system. \nAutomate attendance, benefits, and performance evaluations. \nImprove workforce productivity through smart HR analytics. \nEnsure timely payroll processing and accurate deductions. \nCreate a transparent, employee-friendly HR environment.",
    },
    {
      id: "crm-integration",
      icon: "Leaf",
      title: "CRM Integration",
      subtitle: "Customer-Centric Insights",
      description:
        "Connect ERP and CRM systems for a unified business ecosystem. \nAccess complete customer histories and sales pipelines instantly. \nAutomate lead management and improve retention strategies. \nEmpower teams with synchronized sales and support data. \nDeliver consistent customer experiences across all channels.",
    },
    {
      id: "data-analytics",
      icon: "Leaf",
      title: "Business Intelligence & Analytics",
      subtitle: "Data-Driven Decisions",
      description:
        "Transform ERP data into actionable insights with visual dashboards. \nTrack performance metrics and operational KPIs in real time. \nUse predictive analytics for accurate forecasting and planning. \nEnable decision-makers with customizable reporting tools. \nEnhance strategy with data-backed intelligence across departments.",
    },
    {
      id: "cloud-deployment",
      icon: "Leaf",
      title: "Cloud ERP Deployment",
      subtitle: "Flexible & Scalable Architecture",
      description:
        "Migrate your ERP to a secure and scalable cloud infrastructure. \nAccess data anytime, anywhere with high availability. \nCut infrastructure costs and simplify maintenance tasks. \nEnjoy seamless updates and enhanced disaster recovery. \nEmpower remote teams with reliable, real-time connectivity.",
    },
  ],
},


  managementSolutions: {
    title: "ERP Management & Monitoring Tools",
    subtitle:
      "Our ERP management tools streamline process tracking, data flow, and system optimization for enterprises of any size.",
    subtitleLink: { text: "ERP management platform", url: "#" },
    laptopImage: "/images/management-solutions.jpg",
    mobileImage: "/images/management-solutions-mobile.jpg",
    topFeatures: [
      {
        id: "process-automation",
        icon: "map",
        title: "Process Automation & Workflow Control",
        description:
          "Automate daily business operations. Simplify approvals and task routing. Monitor real-time activities across departments. Reduce manual errors. Improve efficiency and turnaround times.",
      },
      {
        id: "data-analytics",
        icon: "map",
        title: "Data Analytics & Reporting",
        description:
          "Access real-time dashboards and reports. Analyze financials, inventory, and HR performance. Gain insights into business health. Make informed strategic decisions. Improve operational transparency.",
      },
      {
        id: "system-integration",
        icon: "map",
        title: "System Integration & Scalability",
        description:
          "Integrate ERP with CRM, HRM, and other enterprise tools. Ensure seamless data flow and compatibility. Enable modular expansion as your business grows. Support multi-location and multi-currency setups. Enhance overall system scalability.",
      },
    ],
    ctaText: "Get ERP Experts",
    onCtaClick: () => {},
  },

  faq: {
    title: "FAQ - ERP Implementation Services",
    items: [
      {
        id: "1",
        question: "What is ERP implementation?",
        answer:
          "ERP implementation involves deploying and customizing an enterprise resource planning system to centralize and automate business operations across departments.",
      },
      {
        id: "2",
        question: "Which ERP platforms do you support?",
        answer:
          "We work with leading ERP systems such as SAP, Oracle, Microsoft Dynamics, Odoo, and NetSuite, customizing them to fit your business requirements.",
      },
      {
        id: "3",
        question: "How long does ERP implementation take?",
        answer:
          "The timeline depends on your organization's size, modules, and customization level, typically ranging from 3 to 9 months.",
      },
      {
        id: "4",
        question: "Do you provide data migration and training?",
        answer:
          "Yes, we ensure secure data migration from legacy systems and provide hands-on training for users to ensure smooth adoption of the new ERP system.",
      },
      {
        id: "5",
        question: "Can ERP integrate with existing software?",
        answer:
          "Absolutely. We specialize in integrating ERP systems with CRM, HRM, finance, and third-party tools for seamless data exchange and unified operations.",
      },
      {
        id: "6",
        question: "Do you offer cloud-based ERP solutions?",
        answer:
          "Yes, we implement both on-premise and cloud-based ERP systems, ensuring scalability, security, and accessibility for modern enterprises.",
      },
      {
        id: "7",
        question: "Do you provide post-implementation support?",
        answer:
          "Yes, we offer continuous monitoring, maintenance, and system updates to ensure optimal ERP performance and reliability after deployment.",
      },
    ],
  },
};
